...
class transfertomiddleman extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            assetkey: '',
    ...
    onSubmit() {
       ...
       const form = {
        ...
        distributorid: this.state.distributorid
         ...
         var authOptions = {
          method: 'post',
          url: 'http://35.229.19.138:3000/api/org.authentication.whey.transfertomiddleman',
          data: JSON.stringify({"whey": "resource:org.authentication.whey.WheyProtein#"+this.state.assetkey,"middleman": "resource:org.authentication.whey.Middleman#"+this.state.middlemanid,"MiddlemanId":this.state.middlemanid}),
          headers: {
    ...
    render() {
        return (
          ...
          leftComponent={<Icon style={[{color: "white"}]} size={35} name={'ios-arrow-back'} onPress={() => this.props.navigation.goBack()} />}
           centerComponent={{ text: 'TRANSFER TO MIDDLEMAN', style: { color: '#fff' } }}
  ...
  <View style={styles.inputContainer}>
       <TextInput style={styles.inputs}
                ...
                keyboardType="email-address"
                underlineColorAndroid='transparent'
                onChangeText={(middlemanid) => this.setState({middlemanid})}/>
  </View>
  ...
  <TouchableHighlight style={[styles.buttonContainer, styles.loginButton]} onPress={() => this.onSubmit()}>
   ...
   export default transfertomiddleman;
